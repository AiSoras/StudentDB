...
int Point() //Îòâå÷àåò çà '->'
{
	...
	while (1)
	{
		...
		switch (key)
		{
		case 72: //72,75,77,80 - êîä äëÿ êëàâèø ñòðåëîê íà êëàâèàòóðå
			if (c.Y != 1) //Ñòðåëêà ââåðõ
			{
				c.Y--;
				menu--;
			}
			break;
		case 75: //Ñòðåëêà âíèç
			if (c.Y != 1)
			{
				c.Y--;
				menu--;
...
void FirstMenu() //Ïåðâûé óðîâåíü ìåíþ
{
	system("cls");
	fmenu = 1;
	stop = 5;
...
void SecondMenu() //Âòîðîé óðîâåíü ìåíþ
{
	smenu = 1;
	SetConsoleCursorPosition(h, m);
	switch (fmenu)
